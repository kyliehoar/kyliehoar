...
/**
 * Produce a table that counts the number of multiplications/divisions and additions/subtractions
 * within:
 * 
 * (A) MatrixMultiply      [I have done this for you already]
 * (B) Strassen            [You need to instrument the code]
...
public class EmpiricalEvaluation {
	public static void main(String[] args) {
		...
		for (int N : new int[] { 4, 8, 16, 32, 64, 128, 256, 512, 768, 1024, 1280, 1536, 1792, 2048}) {
			...
			if (N < 1024) {
				kjhoar.hw1.Strassen.SMnumMultiplications = 0;
				kjhoar.hw1.Strassen.SMnumAdditions = 0;
				kjhoar.hw1.Strassen.multiply(A, A);
				Strassen.multiply(A, A);
			}
			double timeSM = watch.elapsedTime();
			...
			System.out.println(String.format("%12d %16d %16d %16d %16d %10f %10f", N, 
					...
					kjhoar.hw1.Strassen.SMnumMultiplications/2,           /** Replace with count of Mul/Div for Strassen. */
					kjhoar.hw1.Strassen.SMnumAdditions/2,                 /** Replace with count of Add/Sub for Strassen. */  
					timeMM, timeSM));
		}
	}
}
